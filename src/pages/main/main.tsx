...
const AppWrapper = observer(() => {
    ...
    React.useEffect(() => { const handleSignalLoadAndRun = (event: Event) => { const detail = (event as CustomEvent<{ xml?: string }>).detail; if (!detail?.xml || run_panel.is_running) return; setActiveTab(BOT_BUILDER); let attempts = 0; const applyAndRun = () => { const workspace = window.Blockly?.derivWorkspace; if (!workspace || !window.Blockly || blockly_store.is_loading) { if (attempts++ < 50) window.setTimeout(applyAndRun, 100); return; } try { const BlocklyApi = window.Blockly; const xmlDom = BlocklyApi.utils?.xml?.textToDom ? BlocklyApi.utils.xml.textToDom(detail.xml) : BlocklyApi.Xml.textToDom(detail.xml); workspace.clear(); BlocklyApi.Xml.domToWorkspace(xmlDom, workspace); workspace.render?.(); workspace.cleanUp?.(); workspace.setScale?.(1); window.setTimeout(() => { workspace.zoomToFit?.(); run_panel.onRunButtonClick(); }, 150); } catch (error) { console.error('[VintelFX] Signal AI bot injection failed:', error); } }; window.setTimeout(applyAndRun, 150); }; window.addEventListener('vintelfx-load-and-run-signal-bot', handleSignalLoadAndRun); return () => window.removeEventListener('vintelfx-load-and-run-signal-bot', handleSignalLoadAndRun); }, [blockly_store.is_loading, run_panel, setActiveTab]);
