...
const SignalAI = () => {
    ...
    const buildResults = (ticksByMarket: Record<string, number[]>) => MARKETS
        .map(market => {
            ...
            const preferred = scan_type === 'over_under'
    ...
    const handleScan = () => {
        ...
        ws.onmessage = event => {
            ...
            if (data.error) {
                fail(data.error.message || 'Deriv rejected the market-data request');
                return;
            }
            ...
            if (data.msg_type === 'history' && data.req_id) {
                ...
                market_ticks_ref.current[market.symbol] = prices.map(Number).filter(Number.isFinite).slice(-HISTORY_COUNT);
                received_history += 1;

                if (received_history === MARKETS.length) {
                    finish_initial_scan();
                } else {
                    setStatus(`LIVE · Loading market history ${received_history}/${MARKETS.length}…`);
                }
                return;
            }
            ...
            if (data.msg_type === 'tick' && data.tick?.symbol && data.tick.quote !== undefined) {
                ...
                if (!Number.isFinite(quote) || !MARKETS.some(market => market.symbol === symbol)) return;
                const current = market_ticks_ref.current[symbol] || [];
                market_ticks_ref.current[symbol] = [...current, quote].slice(-HISTORY_COUNT);
                const live_results = buildResults(market_ticks_ref.current);
        ...
        ws.onclose = () => {
            if (!settled) fail('Live Deriv WebSocket closed unexpectedly');
        };

        window.setTimeout(() => {
            if (!settled) fail('Live market connection timed out');
        }, 30000);
    };
