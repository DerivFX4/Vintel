...
const SignalAI = () => {
    ...
    const buildResults = (ticksByMarket: Record<string, number[]>) => {
        ...
        const competitive = candidates.filter(c => best.confidence - c.confidence <= FAIR_MARGIN && best.strength - c.strength <= FAIR_MARGIN);
        ...
        competitive.sort((a, b) => {
            ...
            if (scan_type === 'even_odd') {
                if (a.signal === 'EVEN' && b.signal === 'ODD') return -1;
                if (a.signal === 'ODD' && b.signal === 'EVEN') return 1;
            }
            return b.confidence - a.confidence || b.strength - a.strength;
        });
        const selected = competitive[0] || best;
        shown_signal_counts_ref.current[selected.signal] = (shown_signal_counts_ref.current[selected.signal] || 0) + 1;
